...
Maintained at https://github.com/Pylons/paginate under MIT License
...
class Page(list):
    ...
    def __init__(self, collection, page=1, items_per_page=20, item_count=None,
        ...
        self.item_count = item_count if item_count is not None else len(self.collection)
        ...
        if self.item_count > 0:
            ...
            # Make sure that the requested page number is in the range of valid pages
    ...
    def link_map(self,
        ...
        format:
            Format string that defines how the pager would be normally rendered rendered.
            Uses same arguments as pager()
        ...
        regex_res = re.search(r'~(\d+)~', format)
        self.radius = int(regex_res.group(1) if regex_res else 2)

        # Compute the first and last page number within the radius
        # e.g. '1 .. 5 6 [7] 8 9 .. 12'
        # -> leftmost_page  = 5
        # -> rightmost_page = 9
        leftmost_page = max(self.first_page, (self.page - self.radius)) if \
            self.first_page else None
        rightmost_page = min(self.last_page, (self.page + self.radius)) if \
    ...
    def default_link_tag(item):
        ...
        if not item['href'] or item['type'] in ('span', 'current_page'):
            return str(HTML.span(literal(item['value'] or ''), **item['attrs'])) \
                if item['attrs'] else item['value']
        return str(HTML.a(literal(item['value'] or ''), href=item['href'], **item['attrs']))
